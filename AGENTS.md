...
- `dotnet test DeepSeek.slnx` runs the supported unit and integration test projects for `src/DeepSeek.Core` and `src/Microsoft.Agents.AI.DeepSeek`.
...
- `dotnet pack src/DeepSeek.Core/DeepSeek.Core.csproj` or `dotnet pack src/Microsoft.Agents.AI.DeepSeek/Microsoft.Agents.AI.DeepSeek.csproj` creates NuGet packages in each project's `pack/` folder.
