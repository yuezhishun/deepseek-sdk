...
`src/DeepSeek.Core` contains the core typed SDK, split by API area under `Chat/`, `Completions/`, `Models/`, `Billing/`, `Anthropic/`, and shared helpers in `Internal/`. `src/DeepSeek.Agents.AI` contains the Microsoft.Extensions.AI and Agent Framework adapter layer. Tests live under `test/`, with unit tests in `test/DeepSeek.Tests` and `test/DeepSeek.Agents.AI.UnitTests`, plus live API coverage in `test/DeepSeek.IntegrationTests`. The runnable samples live under `sample/`.
...
- `dotnet test DeepSeek.slnx` runs the supported unit and integration test projects for `src/DeepSeek.Core` and `src/DeepSeek.Agents.AI`.
