...
`src/DeepSeek` contains the core typed SDK, split by API area under `Chat/`, `Completions/`, `Models/`, `Billing/`, `Anthropic/`, and shared helpers in `Internal/`. `src/AgentFramework.AI.DeepSeek` contains the Microsoft.Extensions.AI and Agent Framework adapter layer. Tests live under `test/`, with unit tests in `test/DeepSeek.Tests` and `test/AgentFramework.AI.DeepSeek.Tests`, plus live API coverage in `test/DeepSeek.Next.IntegrationTests`. The runnable sample is `sample/Sample`, and shared test API-key lookup is in `test/TestApiKeyProvider.cs`.
...
- `dotnet test DeepSeek.slnx` runs the unit and integration test projects.
...
## Testing Guidelines
Tests use xUnit. Name test files after the type under test and use method names in the `Method_Scenario_ExpectedResult` style already present, such as `ChatClient_SendsExpectedHeadersPathAndBody`. Keep unit tests deterministic by using in-memory handlers instead of live network calls. Live tests require `sample/appsettings.json` to contain `apiKey`; guard new live cases the same way as `LiveTestGuard.RequireConfigured(...)`.
